...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.agentswitch/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// 存储在本地 ~/.agentswitch/settings.json，不随数据库同步
export interface Settings {
  ...
  // 覆盖 Pi Agent 配置目录（可选）
  piConfigDir?: string;
  // 覆盖 Pi 会话目录（必须为绝对路径，用于相对 sessionDir 无法全局解析的场景）
  piSessionDir?: string;
...
export type McpServerSpecMap = Record<string, McpServerSpec>;

export type McpStorageFormat = "json" | "toml" | "yaml";

export interface AppMcpConfigResponse {
  configPath: string;
  storageFormat: McpStorageFormat;
  servers: McpServerSpecMap;
}

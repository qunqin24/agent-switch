...
/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.agentswitch/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    /// Absolute Pi session directory override used by session management and
    /// usage import when Pi's own `sessionDir` is relative to its launch cwd.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub fn get_pi_session_override_dir() -> Option<PathBuf> {
    let settings = settings_store().read().ok()?;
    let raw = settings.pi_session_dir.as_deref()?.trim();
    if raw == "~" {

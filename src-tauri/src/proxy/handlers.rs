...
/// catalog with a top-level `models` field.  Return the Agent Switch-managed model
...
/// Agent Switch-owned `model_catalog_json`, using the same path ownership rules as
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
    ...
    let active_catalog_path = match crate::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::codex_config::resolve_agentswitch_catalog_path(&config_text, &generated_path)
        }
        Err(_) => None,
    };
        ...
        active_catalog_path.as_ref().filter(|path| path.exists())
    {
        ...
        if active_catalog_path.is_none() {
            log::debug!(
                "[models] stale guard: catalog not served (model_catalog_json not set to Agent Switch catalog)"
            );
        }
        json!({"models": []})
    };
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 Agent Switch 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a Agent Switch limit. ",
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "agentswitch_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "agentswitch_timeout",
        ProxyError::NoAvailableProvider => "agentswitch_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "agentswitch_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "agentswitch_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "agentswitch_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "agentswitch_provider_unhealthy",
        ProxyError::ConfigError(_) => "agentswitch_config_error",
        ProxyError::TransformError(_) => "agentswitch_transform_error",
        ProxyError::InvalidRequest(_) => "agentswitch_invalid_request",
        ProxyError::AuthError(_) => "agentswitch_auth_error",
        ProxyError::UpstreamError { .. } => "agentswitch_upstream_error",
        ProxyError::DatabaseError(_) => "agentswitch_database_error",
        ProxyError::Internal(_) => "agentswitch_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "agentswitch_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert_eq!(body["error"]["code"], "agentswitch_forward_failed");

...
/// Keep the historic header on write so older clients can import snapshots
/// produced by Agent Switch. This is a wire marker, not visible product copy.
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- CC Switch SQLite 导出";
/// Accepted for files written by early Agent Switch rebrand builds.
const AGENTSWITCH_SQL_EXPORT_HEADER: &str = "-- AgentSwitch SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        if trimmed.starts_with(CC_SWITCH_SQL_EXPORT_HEADER)
            || trimmed.starts_with(AGENTSWITCH_SQL_EXPORT_HEADER)
        {
            return Ok(());
        }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("agentswitch.db");
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("agentswitch-periodic-maintenance-backup-disabled-test");

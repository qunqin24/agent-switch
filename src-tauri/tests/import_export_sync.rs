...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".agentswitch").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".agentswitch");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.agentswitch/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("agentswitch-test-invalid<>dir")
    } else {
...
#[test]
fn import_sql_rejects_non_agentswitch_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let import_path = home.join("not-agentswitch.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("non-agentswitch sql should be rejected");
...
#[test]
fn import_sql_accepts_agentswitch_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("agentswitch-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
...
#[test]
fn export_keeps_legacy_header_and_import_accepts_early_agentswitch_header() {
    // 导出继续使用旧 wire header，确保未升级设备可以导入；同时兼容曾经
    // 使用新品牌 header 的早期重命名构建。
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let real_export = state.db.export_sql_string().expect("export should succeed");
    assert!(real_export.starts_with("-- CC Switch SQLite 导出"));
    let agentswitch_export =
        real_export.replacen("-- CC Switch SQLite 导出", "-- AgentSwitch SQLite 导出", 1);
    assert!(agentswitch_export.contains("-- AgentSwitch SQLite 导出"));

    let import_path = home.join("early-agentswitch-export.sql");
    fs::write(&import_path, &agentswitch_export).expect("write Agent Switch import sql");
    ...
    state
        .db
        .import_sql(&import_path)
        .expect("early Agent Switch header should still be accepted");
}

...
"#
    ...
    // Seed a legacy unified MCP row to prove provider switching no longer
    // projects the shared database into the Codex-owned MCP configuration.
    ...
    let new_config_text = new_provider
        ...
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    // Historical provider snapshots remain byte-compatible in storage, but
    // their embedded MCP sections are not projected to the live config.
    assert!(
    ...
    assert!(
        config_text.contains("mcp_servers.legacy"),
        "live config should keep its independently managed MCP servers"
    );
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.agentswitch/settings.json

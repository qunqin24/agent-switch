# Keep Codex Remote Control and Official Plugins While Using Third-Party APIs: AgentSwitch Setup Guide

> Applies to AgentSwitch v3.16.1 and later. This guide is based on the current code, user manual, and v3.16.1 release notes. Screenshots use de-identified sample data and do not include real Access Tokens or API keys.
...
1. In the AgentSwitch Codex panel, switch to `OpenAI Official`.
2. Start Codex and log in once with an official ChatGPT / Codex account. A Free subscription is enough.
3. Return to AgentSwitch and enable `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers`.
...
Open AgentSwitch and switch to the top-level `Codex` tab. First select the `OpenAI Official` provider, or add it from the preset providers if it is missing, and make it the current provider.
...
Return to AgentSwitch and open:
...
If it only supports OpenAI Chat Completions, which is common for DeepSeek / Kimi / MiniMax paths, local routing must be enabled so AgentSwitch can convert Codex Responses requests into Chat Completions requests.
...
After takeover, Codex's live `config.toml` temporarily points to the AgentSwitch local route. The real third-party API key remains in the AgentSwitch provider configuration, and is projected into the `experimental_bearer_token` in `config.toml` when providers are switched.
...
- In AgentSwitch, the current Codex provider is the third-party provider.
...
After `Keep official login when switching third-party providers` is enabled, AgentSwitch takes the third-party provider API key from the provider configuration and writes it under the current provider in `config.toml`:
...
If the provider uses the Chat Completions protocol, AgentSwitch local routing adds another conversion layer:

```text
Codex Responses request
        |
AgentSwitch local route
        |
...
That does not mean model requests are still going to official OpenAI. Actual traffic is determined by the current Codex provider in AgentSwitch, `config.toml`, and local routing logs.
...
Codex reads the model catalog at startup. Even if AgentSwitch has generated a new model catalog, a running Codex process may not hot-load it, so restart Codex after editing model mappings.
...
This is expected. Official account information comes from `auth.json`; the actual model provider comes from `config.toml` and the current provider in AgentSwitch.

**Is a Free subscription really enough?**

Yes. The official account is mainly used to obtain and preserve the official login state required by Codex App. Third-party model requests use the third-party API key configured in AgentSwitch.

**What should I do if official plugins or mobile remote control still do not work?**

Switch back to `OpenAI Official`, restart Codex, and complete official login once. Then confirm `Settings -> General -> Codex App Enhancements -> Keep official login when switching third-party providers` is enabled in AgentSwitch before switching back to the third-party provider.
...
Not recommended. AgentSwitch tries to prevent switching to official providers while local routing takeover is active, because accessing official APIs through a proxy may create account risk. Use official login only to preserve `auth.json`, and route model traffic to third-party providers.
...
- [AgentSwitch v3.16.1 Release Note](../release-notes/v3.16.1-en.md)

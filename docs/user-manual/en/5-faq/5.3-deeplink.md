...
AgentSwitch supports the `agentswitch://` deep link protocol, enabling one-click configuration import via links.
...
AgentSwitch provides an online deep link generator tool:
...
Using AgentSwitch's official online deep link generator tool is more convenient.
...
1. The system asks whether to open AgentSwitch
2. After confirming, AgentSwitch opens
...
AgentSwitch automatically registers the `agentswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "AgentSwitch" --args --register-protocol
```
...
Before import, AgentSwitch will:

...
The current Agent Switch community build is not signed or notarized with an Apple Developer certificate. Download the latest build from the [Releases page](https://github.com/qwq202/agent-switch/releases). If macOS blocks the first launch, open **System Settings > Privacy & Security** and allow the app to run.
...
2. Add Agent Switch to your antivirus software's whitelist
...
chmod +x AgentSwitch-*.AppImage

# If it still fails, try
./AgentSwitch-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.agentswitch/` directory exists
2. Restore from backup: `~/.agentswitch/backups/`
...
**Solutions**:
1. Confirm the file was exported by Agent Switch
...
**Windows**:
- Check taskbar settings to ensure the Agent Switch icon is not hidden
...
2. Restart the app
3. Delete `~/.agentswitch/settings.json` to reset settings
...
3. Download the newest installer from this repository's [Releases page](https://github.com/qwq202/agent-switch/releases)

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and Agent Switch runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, Agent Switch covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `agentswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/qwq202/agent-switch/issues)
...
- macOS/Linux: `~/.agentswitch/logs/`
- Windows: `%APPDATA%\agentswitch\logs\`

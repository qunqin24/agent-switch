...
AgentSwitch automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with AgentSwitch configuration
[Expand] [Dismiss]
```
...
3. AgentSwitch will automatically back up and remove the selected variables
...
If you prefer not to use AgentSwitch to remove variables, you can handle them manually:
...
- AgentSwitch provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use AgentSwitch to manage configurations**: Avoid setting API keys in system environment variables

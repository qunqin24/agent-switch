...
Hermes uses YAML configuration. AgentSwitch writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to AgentSwitch's "providers") |
...
AgentSwitch's priority when modifying configurations:

1. **AgentSwitch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by AgentSwitch)
- AgentSwitch's `settings.json`
...
AgentSwitch v3.7.0 migrated from JSON files to SQLite:

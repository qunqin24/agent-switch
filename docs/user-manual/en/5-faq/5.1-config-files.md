...
## Agent Switch Data Storage

### Storage Directory

Default location: `~/.agentswitch/`
...
```
~/.agentswitch/
├── agentswitch.db      # SQLite database (SSOT)
...
`agentswitch.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. Agent Switch writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to Agent Switch's "providers") |
...
Agent Switch's priority when modifying configurations:

1. **Agent Switch Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by Agent Switch)
- Agent Switch's `settings.json`

### Not Recommended to Edit Manually

- `agentswitch.db` database file
- Backup files
...
Agent Switch v3.7.0 migrated from JSON files to SQLite:

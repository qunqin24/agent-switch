...
This section describes how to configure AgentSwitch according to your preferences.
...
AgentSwitch supports four languages:
...
When enabled, AgentSwitch automatically runs when the system starts.
...
Starting from v3.13.0, AgentSwitch adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger AgentSwitch via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If AgentSwitch runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, AgentSwitch automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that AgentSwitch uses when opening a terminal.
...
The storage location for AgentSwitch's own data, defaulting to `~/.agentswitch/`.
...
Configure AgentSwitch's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, AgentSwitch automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
- AgentSwitch automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, AgentSwitch displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current AgentSwitch version number, with support for:

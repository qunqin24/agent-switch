...
This section describes how to configure Agent Switch according to your preferences.
...
Agent Switch supports four languages:
...
When enabled, Agent Switch automatically runs when the system starts.
...
Starting from v3.13.0, Agent Switch adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger Agent Switch via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If Agent Switch runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, Agent Switch automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that Agent Switch uses when opening a terminal.
...
The storage location for Agent Switch's own data, defaulting to `~/.agentswitch/`.
...
The exported file name format is `agentswitch-export-{timestamp}.sql`.
...
Configure Agent Switch's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, Agent Switch automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `agentswitch-sync`) |
...
- Agent Switch automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, Agent Switch displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current Agent Switch version number, with support for:

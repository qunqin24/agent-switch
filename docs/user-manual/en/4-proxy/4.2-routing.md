...
App routing means letting AgentSwitch route a specific application's API requests through the local routing service.
...
When routing is enabled, AgentSwitch modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, AgentSwitch will:
...
Before enabling routing, AgentSwitch backs up the original configuration and restores it when disabled.
